...
Define bins of equal population size by defining evenly-spaced quantiles of the distribution over activity levels (up to some maximum quantile less than 1) and set the activity level for each bin to its mean, computed by numerical integration via one dimensional adaptive quadrature over the activity range of the bin.
...
![Placement of bins by the two schemes relative to the target distribution, using 7 bins and $\sigma = 0.5$. (A) the target lognormal density of activity levels. (B, C) the bin activity levels (points) and population fractions (vertical stems) for the quantile mean (B) and Gaussian quadrature (C) schemes respectively.](../figures/binning_illustration.png){#fig-binning-illustration}
